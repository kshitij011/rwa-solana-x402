          ...
          {steps.map((step, index) => (
            ...
            >
              ...
              {index < steps.length - 1 && (
                <div className="hidden lg:block absolute top-12 left-full w-full h-px bg-gradient-to-r from-border via-gold/30 to-border z-0" />
              )}
              
              <div className="relative z-10 p-6 rounded-2xl bg-gradient-card border border-border transition-all duration-300 hover:border-gold/30 hover:shadow-gold-glow/5">
                {/* Step Number */}
                <div className="absolute -top-3 -right-3 h-8 w-8 rounded-full bg-gold flex items-center justify-center text-sm font-bold text-primary-foreground">
                  {index + 1}
                </div>
